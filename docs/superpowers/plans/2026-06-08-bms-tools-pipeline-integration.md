...
def test_group_leaf_names_returns_children_in_order() -> None:
    grp = DataLayout(
        fields={
            ...
            "STATMSGO": FieldLayout(name="STATMSGO", type_descriptor=_alpha(78), offset=20, byte_length=78),
        },
        offset=0,
        total_bytes=98,
    )
    ...
    assert mat.group_leaf_names("COSGN0AO") == ["USERIDO", "STATMSGO"]
...
def test_send_map_reads_named_output_fields_from_ws(monkeypatch) -> None:
    # WS layout: USERIDO at 0 len 8, STATMSGO at 8 len 78.
    layout = {
        "USERIDO": {"offset": 0, "length": 8},
        "STATMSGO": {"offset": 8, "length": 78},
    }
    ...
    builtin([_tv("COSGN0A"), _tv(["USERID", "STATMSG"])], vm)  # (map_name, base_names)
    item = sq.get_nowait()
    assert item["map"] == "COSGN0A"
    assert item["fields"]["USERID"] == "USER0001"
    assert item["fields"]["STATMSG"] == ""  # spaces -> rstripped empty
```
...
def test_send_map_lowers_from_group_field_names():
    # Layout has group COSGN0AO with leaves USERIDO, STATMSGO.
    ...
    insts = _lower_exec_cics(  # helper provided by this test module's harness
        ...
        group_leaf_names={"COSGN0AO": ["USERIDO", "STATMSGO"]},
    )
    ...
    assert const.value == ["USERID", "STATMSG"]
```
