...
step produces a cascading `SymbolicValue`. In [REDACTED] this creates
...
2. **Project classes** — auto-populated from `compile_directory()`. The
   registry already knows `com.test.[REDACTED]` exists, so it gets
   registered as `com → test → [REDACTED](type)`.
...
1. **Phase 1**: [REDACTED] "java" symbolics drop from 67 to ≤19
