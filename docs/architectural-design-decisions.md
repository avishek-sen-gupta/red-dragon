...
### ADR-126: Eliminate IRInstruction — full Register/Label type safety (2026-03-24)
...
### ADR-127: Domain type migration campaign — BinopKind, VarName, FieldName, FuncName (2026-03-26 to 2026-03-29)
...
### ADR-128: Address domain type — heap and region keys (2026-03-29)

**Context:** After the domain type migration campaign (ADR-127) replaced all instruction field strings with typed wrappers, the heap (`VMState.heap`) and regions (`VMState.regions`) still used bare string keys (`"obj_0"`, `"arr_3"`, `"mem_0"`). These strings were generated by `fresh_symbolic()` and passed through `Pointer.base`, `HeapWrite.obj_addr`, `NewObject.addr`, and dozens of handler sites. A bare string key could silently match a variable name, field name, or any other string — no type-level distinction.

**Decision:** Introduce `Address` (`interpreter/address.py`) as a frozen dataclass wrapping a `str` value. No `__eq__(str)` bridge — strict from day one. Migrate `Pointer.base`, `HeapWrite.obj_addr`, `NewObject.addr`, `RegionWrite.addr`, and all `VMState` heap/region dict keys to `Address`. Privatize the heap and regions behind accessor methods.

**Implementation:**
1. Introduce `Address` dataclass, `NO_ADDRESS` sentinel, `NullHeapObject` null-object sentinel
2. Add `VMState` accessor methods: `heap_get(addr)`, `heap_set(addr, obj)`, `heap_contains(addr)`, `heap_ensure(addr)`, `heap_items()`, `heap_keys()`, `heap_count()`, `heap_values()`, and corresponding `region_*` methods
3. Migrate all direct `vm.heap[addr]` and `vm.regions[addr]` accesses to use accessors
4. Make `_heap` and `_regions` private (underscore prefix)
5. `heap_get` returns `NO_HEAP_OBJECT` (null object) for missing addresses — eliminates `KeyError` and `None` checks

**Consequences:** Heap addresses are type-safe — an `Address` cannot be confused with a `VarName`, `FieldName`, or bare string. The accessor pattern encapsulates storage and enables future changes (e.g., garbage collection, copy-on-write) without touching callers. ~200 changes across ~30 files, 13,100+ tests passing.

**Key files:** `interpreter/address.py`, `interpreter/vm/vm_types.py`.

---

### ADR-129: StorageIdentifier protocol and reads()/writes() on instructions (2026-03-30)

**Context:** The dataflow module (`interpreter/dataflow.py`) used opcode-conditional logic to determine which registers and variables an instruction reads from and writes to. This logic was duplicated in `block_facts()`, `extract_def_use_chains()`, and interprocedural analysis. Each new opcode required updating these conditionals in multiple places.

**Decision:** Define a `StorageIdentifier` runtime-checkable `Protocol` (`interpreter/storage_identifier.py`) requiring a `name` property. Both `Register` and `VarName` implement it. Add `reads()` and `writes()` methods to `InstructionBase`, overridden by each per-opcode subclass to return `list[StorageIdentifier]` — the set of storage locations read from and written to.
...
**Context:** Java hex literals like `0x7f` were stored as raw strings in the IR. When the VM compared `i < "0x7f"`, it couldn't evaluate the comparison concretely, producing a symbolic value that cascaded through all downstream operations. In [REDACTED], a single `i < 0x7f` loop guard produced 3,430+ cascading symbolics.
...
**Consequences:** Hex/octal/binary literals produce concrete integer values in the VM. The `i < 0x7f` cascade in [REDACTED] is fully eliminated. 13,267 tests passing.
...
**Context:** Fully-qualified Java references like `java.util.Arrays.fill(arr, val)` are lowered as a `LOAD_VAR "java"` → `LOAD_FIELD "util"` → `LOAD_FIELD "Arrays"` → `CALL_METHOD "fill"` chain, producing 4 cascading symbolic values per chain (67 total in [REDACTED]). Tree-sitter parses these as nested `field_access` nodes in expression position — indistinguishable from real field access at the syntax level.
...
**Amendment (2026-09-15, ADR-148/ADR-149):** `__cobol_round` now receives an exact `CobolNumber` (or numeric text) and delegates to `cobol_numeric.round_half_up`; its `decimal` import moved into `cobol_numeric`. `force_division_float` is gone — a ROUNDED receiver instead contributes its decimal places **plus one** to IBM's `dmax`, so the quotient carries the extra digit the rounding needs.

---

### ADR-147: COBOL STOP RUN — dedicated `Halt_` instruction distinct from `Return_` (2026-07-03)

**Context:** As established in ADR-145, `STOP RUN`, `GOBACK`, and `EXIT PROGRAM` all lowered to the same `Return_` IR instruction. The VM's `_handle_return_flow` (`interpreter/run.py:286-322`) treats every `Return_` uniformly — pop exactly one call-stack frame and resume at the caller (halting only if the popped frame is the top-level/`MAIN_FRAME_NAME` frame or the stack is empty). This meant `STOP RUN` inside a called subprogram incorrectly returned control to the caller instead of terminating the entire run unit — the opposite of correct COBOL semantics, where `STOP RUN` must unconditionally halt from any call depth and only `GOBACK`/`EXIT PROGRAM` should return to a caller.

**Decision:** Two designs were considered:
- **A boolean flag on `Return_`** (e.g. `halt_all: bool`) — smaller diff, but rejected because return-type inference (`interpreter/types/type_inference.py`, `_infer_return`) dispatches on exact instruction type, not field values; a flagged `Return_` would still have its synthetic value incorrectly unioned into the enclosing function's inferred return type, even though `STOP RUN` never actually returns anything to anyone.
- **A dedicated `Halt_` instruction (chosen)** — add `Opcode.HALT` to the `Opcode` enum (`interpreter/ir.py`, after `THROW`) and a `Halt_` dataclass mirroring `Return_`'s shape but with no value/result (`interpreter/instructions.py`). `interpreter/handlers/control_flow.py` gets `_handle_halt`, registered under `Opcode.HALT` in `interpreter/vm/executor.py`'s dispatch table. COBOL's `lower_stop_run` (`interpreter/cobol/lower_arithmetic.py`) now emits `Halt_()` directly, dropping the synthetic `zero_reg`/`Const.int_` that `Return_` required. `GOBACK`/`EXIT PROGRAM` are unchanged and remain `Return_`.

Because `Halt_` is a distinct type, it is naturally absent from `_infer_return`'s dispatch table — no code change needed there — and is more semantically honest: a halt is not a return. The VM step loop, at both the plain and traced-execution sites (`interpreter/run.py`, near lines 444 and 791), treats `Halt_` as an unconditional `break` placed before the existing `is_return`/`is_throw` handling, bypassing `_handle_return_flow` entirely — no frame pop, no caller-register write, no `MAIN_FRAME_NAME` check. `interpreter/cfg.py` adds `Halt_` alongside `Return_`/`Throw_` in both the block-start-detection tuple (line ~38) and the no-successors terminator check (line ~89), so a block ending in `Halt_` is correctly treated as having no fallthrough successor.

**Consequences:** `STOP RUN` now correctly terminates the whole run unit from any call depth. This is the first VM instruction that intentionally does not resume any caller when a call frame is torn down, and it surfaced a latent architectural coupling: COBOL's `CALL ... USING BY REFERENCE` parameter copy-back (ADR-144) is caller-emitted IR that only executes if control resumes at the caller via the normal return-flow path (`_handle_return_flow` in `interpreter/run.py`). Since `Halt_` deliberately never resumes the caller, a BY REFERENCE write made by a subprogram that then executes `STOP RUN` (instead of `GOBACK`/`EXIT PROGRAM`) is now silently lost — filed as `red-dragon-zerg`, deferred as its own follow-up since a proper fix requires moving copy-back semantics from caller-emitted IR into VM-level frame-teardown handling, a bigger change than `STOP RUN`'s own scope. `Halt_` is COBOL-only; no other of the 15+ language frontends emit it.

---

### ADR-148: COBOL exact fixed-point arithmetic behind a `cobol_numeric` boundary (2026-09-15)

**Context:** COBOL fixed-point values were decoded, computed and re-encoded through Python `float` (red-dragon-4q25.1). Verified losses: `COMPUTE X = 4.35 * 100` into `PIC 999` stored 434; an 18-digit `ADD` lost its last digit; COMP/BINARY fields with decimal places lost their fraction on decode AND their scaling on store (red-dragon-0dvs, both halves). A guard-and-truncate step (96651c84) masked some float noise but rounded genuine values up (red-dragon-5b93). The two numeric encode paths also truncated fractions differently (red-dragon-bqds), the second such divergence in that pair after red-dragon-qhtv.

**Decision:** A new top-level leaf package `cobol_numeric` owns the exact representation (`CobolNumber`, currently Python `Decimal`) and every conversion between it and COBOL representations: stored digits (`from_digits`, `digits_for_encode`), PIC P scaling (`scale_by`), literals (`from_literal`), IEEE floats (`from_float`/`to_float`), text (`to_plain_str`, whose `-?digits(.digits)?` grammar `IS NUMERIC` relies on), truncation and ROUNDED, and intrinsic-function math. Arithmetic uses exact integer ratios, never the `decimal` context — whose default 28-digit precision would round IBM's 30-digit intermediates (verified: `123456789.123456789 × 987654321.987654321` loses its last 8 digits). COBOL lowering emits boundary builtins (`__cobol_add/subtract/multiply/divide`, `__cobol_from_digits`, `__cobol_scale_by`, `__cobol_parse_number`, `__cobol_binary_unscaled`, `__cobol_to_text`, `__cobol_to_float`). A `Decimal` foundation type and `Const.decimal_` are the only shared-code additions. Scope is COBOL only. Rejected: scaled integers with statically tracked scale (rewrites all COBOL lowering; consumers see raw scaled ints) and `fractions.Fraction` (non-COBOL intermediate semantics, unbounded growth).

**Consequences:** Other languages' conversion rules, binop coercion, `BINOP_TABLE` and `str` builtin are unchanged. An import-linter contract (`cobol-numeric-is-a-leaf`) and an invariant test (no `decimal` outside `cobol_numeric`) hold the boundary, so replacing the representation means editing one package. Both numeric encode paths share one `encode_digits`, ending the qhtv/bqds divergence class, and both now take the sign from the pre-truncation value — whether a negative value truncating to zero should store 0xD or 0xC is unverified against a real compiler and tracked as red-dragon-8u6x. Closes red-dragon-4q25.1, red-dragon-0dvs, red-dragon-bqds and red-dragon-5b93. Exponentiation (`**`) remains unlowered here (red-dragon-wxur).
...
**Decision:** IBM's places-carried table lives as pure `Scale` functions in `cobol_numeric.scale` (`+ -`: `max(i1, i2) + 1` / `max(d1, d2)`; `*`: `i1 + i2` / `d1 + d2`; `/`: `i2 + d1` / `max(d2 − d1, dmax)`, where op1 is the divisor), with the 30-digit carry table in `carry`. `interpreter/cobol/arithmetic_scale.py` computes each expression node's `Scale` and each statement's `dmax` statically from PIC clauses, literals and receivers — a ROUNDED receiver contributing its decimal places plus one — and lowering passes the carried decimal places to the exact builtin as a constant. IBM's floating-point rule applies per expression: a COMP-1/COMP-2 operand or receiver, or a floating intrinsic, makes the whole expression IEEE floating point. `force_division_float` and COBOL's reliance on `(Int, Int) /` → `//` are removed; the 18-digit PIC limit is retained.

**Consequences:** Integer-only expressions truncate exactly as before; receivers with decimal places now keep the fractions IBM keeps (`7 / 2 * 2` into `PIC 9V9` = 7.0, previously 6.0). The ROUNDED + 1 rule was verified against NIST-85 (`NC117A.CBL`: `DIVIDE 4 INTO WRK-DS-TS-12V00-S ROUNDED` with the field holding 2 must give 1, which needs the quotient to carry a decimal place). The carry table's high-order integer truncation is not emulated — under the 18-digit PIC limit a receiver overflows first, and `ON SIZE ERROR` catches that. The intrinsic classification (floating / integer / argument-scaled) is this project's reading of IBM's function types. `ARITH(EXTEND)` is a `limit=31` extension of `carry`; GnuCOBOL-style division is out of scope.
