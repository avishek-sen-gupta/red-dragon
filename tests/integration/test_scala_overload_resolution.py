"""Integration tests for Scala overload resolution through full pipeline.

Tests that when Scala source code defines overloaded methods, the executor picks
...
class TestScalaOverloadResolutionByArity:
    """Scala methods overloaded by parameter count."""
...
class TestScalaOverloadResolutionByType:
    """Scala methods overloaded by parameter type (same arity)."""

    def test_int_vs_string_picks_int(self):
        source = """\
class Printer {
  def show(x: Int): Int = x + 1
  def show(s: String): String = "str:" + s
}
val p = new Printer()
