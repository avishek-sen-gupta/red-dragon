...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "Turn 5 advances past the prior MOVE gap (red-dragon-0fqr) and the "
        "relation-compare gap (red-dragon-dmu8, now fixed: a numeric-DISPLAY "
        "field vs an alphanumeric figurative/char-literal compares by its zoned "
        "display representation). The ENTER input IS delivered and all relation "
        "conditions in 2200-EDIT-MAP-INPUTS evaluate correctly (verified: "
        "ACCTSIDI decodes to 11 / display '00000000011'; ACCTSIDI='*' -> False, "
        "ACCTSIDI=SPACES -> False, CC-ACCT-ID=LOW-VALUES/SPACES/ZEROS -> all "
        "False). It is blocked on a DIFFERENT gap: the level-88 condition-name "
        "FLG-ACCTFILTER-BLANK (88 VALUE ' ' on WS-EDIT-ACCT-FLAG PIC X(1)) "
        "evaluates TRUE even after 2210-EDIT-ACCOUNT does SET "
        "FLG-ACCTFILTER-ISVALID TO TRUE ('1'). The 88 parent decodes to THREE "
        "spaces ('   '), not the field's single byte — WS-EDIT-ACCT-FLAG is the "
        "3rd of three adjacent PIC X(1) flags (WS-INPUT-FLAG, WS-PFK-FLAG, "
        "WS-EDIT-ACCT-FLAG) and the 88 parent-field resolution reads a 3-byte "
        "span (or the SET writeback to the single byte is not reflected), so "
        "FLG-ACCTFILTER-BLANK ('   ' == ' ') is TRUE. That drives "
        "1300-SETUP-SCREEN-ATTRS (MOVE '*' TO ACCTSIDO) and "
        "SET NO-SEARCH-CRITERIA-RECEIVED, rendering STATMSG='No input received' "
        "and ACCTSID='*', so 9000-READ-ACCT's three chained VSAM reads never "
        "run. This is a COBOL 88-level condition-name parent width / SET "
        "writeback gap on adjacent single-byte flag fields, NOT a relation "
        "compare or input-delivery issue. The dmu8 fix is independently proven "
        "by tests/integration/test_cobol_programs.py::"
        "TestNumericDisplayVsAlphanumericRelation."
    ),
)
@covers(CobolFeature.EXEC_CICS, CobolFeature.INTRINSIC_FUNCTION)
