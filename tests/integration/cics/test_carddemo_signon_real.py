...
with an in-memory USRSEC dataset seeded with one user; the BMS symbolic maps are
generated by the bms-tools pipeline (gated on BMS_TOOLS_HOME) and resolved as
ordinary copybooks. Asserts the dispatcher result is an XCTL to the menu program.
"""
...
def test_real_carddemo_signon_menu_and_option_select(tmp_path):
    """Real four-turn flow through unmodified CardDemo source:
    ...
    All three programs share one CicsLoweringStrategy (USRSEC engine + BMS maps).
