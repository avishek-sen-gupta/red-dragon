...
def _run_c(source: str, max_steps: int = 500) -> dict:
    return run_locals(source, Language.C, max_steps)
...
int answer = p.x + p.y;
""")
        assert vars_[VarName("answer")] == 52
...
""")
        assert (
            isinstance(vars_[VarName("result")], int) and vars_[VarName("result")] == 5
        )
...
int sum = p.x + p.y;
""")
        assert isinstance(vars_[VarName("sum")], int) and vars_[VarName("sum")] == 30
...
""")
        assert (
            isinstance(vars_[VarName("result")], int) and vars_[VarName("result")] == 7
        )
