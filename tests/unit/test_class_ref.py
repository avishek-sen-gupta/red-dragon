...
class TestClassRef:
    def test_construction_no_parents(self):
        ref = ClassRef(name="Dog", label=CodeLabel("class_Dog_0"), parents=())
        assert ref.name == "Dog"
        assert ref.label == "class_Dog_0"
        assert ref.parents == ()

    def test_construction_with_parents(self):
        ref = ClassRef(name="Dog", label=CodeLabel("class_Dog_0"), parents=("Animal",))
        assert ref.parents == ("Animal",)

    def test_multiple_parents(self):
        ref = ClassRef(name="C", label=CodeLabel("class_C_0"), parents=("A", "B"))
    ...
    def test_equality(self):
        a = ClassRef(name="Dog", label=CodeLabel("class_Dog_0"), parents=("Animal",))
        b = ClassRef(name="Dog", label=CodeLabel("class_Dog_0"), parents=("Animal",))
        assert a == b

    def test_different_labels_not_equal(self):
        a = ClassRef(name="Dog", label=CodeLabel("class_Dog_0"), parents=())
        b = ClassRef(name="Dog", label=CodeLabel("class_Dog_1"), parents=())
        assert a != b
