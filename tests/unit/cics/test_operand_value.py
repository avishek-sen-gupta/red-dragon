...
def test_field_operand_emits_decode_not_literal_const() -> None:
    ctx = FakeCtx({"WS-TRANID": (0, 4)})
    out = emit_operand_value(ctx, "WS-TRANID", MATERIALISED)
    ...
    assert not any(isinstance(i, Const) and i.value == "WS-TRANID" for i in ctx.emitted)


@covers(NotLanguageFeature.INFRASTRUCTURE)
def test_literal_operand_emits_const_with_quotes_stripped() -> None:
    ctx = FakeCtx({})
    out = emit_operand_value(ctx, "'CC01'", MATERIALISED)
    assert isinstance(out, Register)
    assert ctx.decoded == []
