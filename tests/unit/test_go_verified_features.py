# pyright: standard
"""Enforcement tests for the Go verified-features registry.

For every entry in GO_VERIFIED_FEATURES, asserts that each referenced
...
from interpreter.frontends.go.verified_features import (
    GO_VERIFIED_FEATURES,
    VerifiedFeature,
)
...
def _all_refs() -> list[tuple[VerifiedFeature, str, str]]:
    return [
        (feat, mod, cls) for feat in GO_VERIFIED_FEATURES for mod, cls in feat.test_refs
    ]
