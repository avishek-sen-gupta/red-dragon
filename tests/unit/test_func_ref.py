...
class TestFuncRef:
    ...
    def test_equality(self):
        a = FuncRef(name="add", label=CodeLabel("func_add_0"))
        b = FuncRef(name="add", label=CodeLabel("func_add_0"))
        assert a == b

    def test_different_labels_not_equal(self):
        a = FuncRef(name="add", label=CodeLabel("func_add_0"))
        b = FuncRef(name="add", label=CodeLabel("func_add_1"))
        assert a != b

    def test_dotted_name(self):
        """Dotted names like Counter.new are valid — the whole point of this refactor."""
        ref = FuncRef(name="Counter.new", label=CodeLabel("func_new_0"))
        assert ref.name == "Counter.new"


class TestBoundFuncRef:
    def test_construction_with_closure(self):
        fr = FuncRef(name="inner", label=CodeLabel("func_inner_0"))
    ...
    def test_construction_without_closure(self):
        fr = FuncRef(name="add", label=CodeLabel("func_add_0"))
