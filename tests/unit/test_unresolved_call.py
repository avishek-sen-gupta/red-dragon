...
def _make_call_inst(
    result_reg: Register = Register("%0"), func_name: str = "unknown_func"
) -> InstructionBase:
...
def _make_method_inst(result_reg: Register = Register("%0")) -> InstructionBase:
...
class TestLLMPlausibleResolver:
    ...
    def test_resolve_call_with_side_effects(self):
        ...
        assert result.update.heap_writes[0].obj_addr == Address("arr_5")
