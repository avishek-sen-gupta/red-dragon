...
class TestBuildExportTable:
    ...
    def test_exports_functions(self):
        func_table = {
            "func_helper_0": FuncRef(name="helper", label="func_helper_0"),
            "func_main_2": FuncRef(name="main", label="func_main_2"),
        }
    ...
    def test_exports_classes(self):
        class_table = {
            "class_User_0": ClassRef(name="User", label="class_User_0", parents=()),
        }
    ...
    def test_skips_variables_inside_functions(self):
        ...
        ir = [
            IRInstruction(opcode=Opcode.LABEL, label="entry"),
            IRInstruction(opcode=Opcode.LABEL, label="func_helper_0"),
            IRInstruction(opcode=Opcode.STORE_VAR, operands=["local_var", "%0"]),
            IRInstruction(opcode=Opcode.LABEL, label="end_helper_1"),
        ]
        et = build_export_table(ir, {}, {})
        assert et.variables == {}
    ...
    def test_skips_variables_inside_classes(self):
        ...
        ir = [
            IRInstruction(opcode=Opcode.LABEL, label="entry"),
            IRInstruction(opcode=Opcode.LABEL, label="class_User_0"),
            IRInstruction(opcode=Opcode.STORE_VAR, operands=["field", "%0"]),
            IRInstruction(opcode=Opcode.LABEL, label="end_class_User_1"),
        ]
        et = build_export_table(ir, {}, {})
        assert et.variables == {}
