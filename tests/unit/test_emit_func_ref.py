...
class TestEmitFuncRef:
    def test_registers_in_symbol_table(self):
        ctx = _make_ctx()
        ctx.emit_func_ref("add", "func_add_0", result_reg=Register("%0"))
    ...
    def test_plain_label_format(self):
        ctx = _make_ctx()
        ctx.emit_func_ref("my_func", "func_my_func_0", result_reg=Register("%1"))
    ...
    def test_multiple_registrations(self):
        ctx = _make_ctx()
        ctx.emit_func_ref("foo", "func_foo_0", result_reg=Register("%0"))
        ctx.emit_func_ref("bar", "func_bar_0", result_reg=Register("%1"))
    ...
    def test_dotted_name_works(self):
        ...
        ctx = _make_ctx()
        ctx.emit_func_ref("Counter.new", "func_new_0", result_reg=Register("%0"))
