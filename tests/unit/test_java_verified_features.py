# pyright: standard
"""Enforcement tests for the Java verified-features registry.

For every entry in JAVA_VERIFIED_FEATURES, asserts that each referenced
...
from interpreter.frontends.java.verified_features import (
    JAVA_VERIFIED_FEATURES,
    VerifiedFeature,
)
...
def _all_refs() -> list[tuple[VerifiedFeature, str, str]]:
    return [
        (feat, mod, cls)
        for feat in JAVA_VERIFIED_FEATURES
