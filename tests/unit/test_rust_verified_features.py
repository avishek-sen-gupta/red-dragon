# pyright: standard
"""Enforcement tests for the Rust verified-features registry.

For every entry in RUST_VERIFIED_FEATURES, asserts that each referenced
...
from interpreter.frontends.rust.verified_features import (
    RUST_VERIFIED_FEATURES,
    VerifiedFeature,
)
...
def _all_refs() -> list[tuple[VerifiedFeature, str, str]]:
    return [
        (feat, mod, cls)
        for feat in RUST_VERIFIED_FEATURES
