"""Exercism test: rna-transcription across all 15 deterministic frontends.

Uses canonical test data from Exercism problem-specifications.
Each language solution is a separate file under exercises/rna_transcription/solutions/.
"""
...
EXERCISE = "rna_transcription"

SNAKE_CASE_LANGUAGES = frozenset({"python", "ruby", "rust"})


def _function_name(language: str) -> str:
    """Return the rna-transcription function name for *language*."""
    return "to_rna" if language in SNAKE_CASE_LANGUAGES else "toRna"
...
class TestRnaTranscriptionLowering:
...
class TestRnaTranscriptionCrossLanguage:
...
class TestRnaTranscriptionExecution:
    @pytest.fixture(
        params=[
            (lang, case)
