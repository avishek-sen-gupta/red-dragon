"""Exercism test: perfect-numbers across all 15 deterministic frontends.

Uses canonical test data from Exercism problem-specifications.
Each language solution is a separate file under exercises/perfect_numbers/solutions/.
"""
...
def _function_name(language: str) -> str:
    """Return the classify function name for *language*."""
    return "classify"
...
CANONICAL_CASES: list[dict] = [
    case for case in load_canonical_cases(EXERCISE) if case["input"]["number"] <= 10000
]
...
class TestPerfectNumbersLowering:
...
class TestPerfectNumbersCrossLanguage:
...
class TestPerfectNumbersExecution:
    @pytest.fixture(
        params=[
            (lang, case)
