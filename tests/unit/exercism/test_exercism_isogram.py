"""Exercism test: isogram across all 15 deterministic frontends.

Uses canonical test data from Exercism problem-specifications.
Each language solution is a separate file under exercises/isogram/solutions/.

Uses a toLowerChar helper function with 26 if-statements for
case-insensitive comparison, since the VM has no lower()/ord() builtins.
"""
...
def _function_name(language: str) -> str:
    """Return the isogram function name for *language*."""
    return "is_isogram" if language in SNAKE_CASE_LANGUAGES else "isIsogram"
...
class TestIsogramCrossLanguage:
