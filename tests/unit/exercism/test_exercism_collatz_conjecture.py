"""Exercism test: Collatz conjecture across all 15 deterministic frontends.

Uses canonical test data from Exercism problem-specifications.
Each language solution is a separate file under exercises/collatz_conjecture/solutions/.
"""
...
EXERCISE = "collatz_conjecture"

SNAKE_CASE_LANGUAGES = frozenset({"python", "ruby", "rust"})


def _function_name(language: str) -> str:
    """Return the collatz function name for *language*."""
    return "collatz_steps" if language in SNAKE_CASE_LANGUAGES else "collatzSteps"
...
class TestCollatzCrossLanguage:
