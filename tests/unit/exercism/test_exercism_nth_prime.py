"""Exercism test: nth-prime across all 15 deterministic frontends.

Uses canonical test data from Exercism problem-specifications.
Each language solution is a separate file under exercises/nth_prime/solutions/.

Cases where input number > 100 are filtered out to keep VM step count
manageable (~16M steps for the 10001st prime).
"""
...
def _function_name(language: str) -> str:
    """Return the nth-prime function name for *language*."""
    return "nth_prime" if language in SNAKE_CASE_LANGUAGES else "nthPrime"
...
CANONICAL_CASES: list[dict] = [
    case for case in load_canonical_cases(EXERCISE) if case["input"]["number"] <= 100
]
...
class TestNthPrimeCrossLanguage:
