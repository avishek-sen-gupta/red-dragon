"""Exercism test: acronym across all 15 deterministic frontends.

Uses canonical test data from Exercism problem-specifications.
Each language solution is a separate file under exercises/acronym/solutions/.

Uses a toUpperChar helper function with 26 if-statements for
uppercasing, and word boundary detection (space, hyphen, underscore
are separators; apostrophe is NOT a separator).

Pascal is excluded from execution tests because the apostrophe in
"Halley's Comet" triggers the known Pascal string escaping limitation
(ADR-024).
"""
...
class TestAcronymCrossLanguage:
