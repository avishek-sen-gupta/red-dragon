"""Exercism test: hamming distance across all 15 deterministic frontends.

Uses canonical test data from Exercism problem-specifications.
Each language solution is a separate file under exercises/hamming/solutions/.
"""
...
def _function_name(language: str) -> str:
    """Return the hamming-distance function name for *language*."""
    return "hamming_distance" if language in SNAKE_CASE_LANGUAGES else "hammingDistance"
...
class TestHammingCrossLanguage:
...
def _case_args(case: dict) -> list[object]:
    strand1 = case["input"]["strand1"]
    strand2 = case["input"]["strand2"]
    return [strand1, strand2, len(strand1)]
