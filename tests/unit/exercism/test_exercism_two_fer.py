"""Exercism test: two-fer across all 15 deterministic frontends.

Uses canonical test data from Exercism problem-specifications.
Each language solution is a separate file under exercises/two_fer/solutions/.
"""
...
def _function_name(language: str) -> str:
    """Return the two-fer function name for *language*."""
    return "two_fer" if language in SNAKE_CASE_LANGUAGES else "twoFer"
...
class TestTwoFerDefaultParameter:
    ...
    def test_no_arg_uses_default(self, lang):
        _DEFAULT_PARAM_SUPPORTED = {"python", "javascript", "typescript", "ruby", "cpp"}
