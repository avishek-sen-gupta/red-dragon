"""Exercism test: resistor-color across all 15 deterministic frontends.

Uses canonical test data from Exercism problem-specifications.
Each language solution is a separate file under exercises/resistor_color/solutions/.

Only the colorCode property is tested (colors returns an array, unsupported).
"""
...
EXERCISE = "resistor_color"

SNAKE_CASE_LANGUAGES = frozenset({"python", "ruby", "rust"})


def _function_name(language: str) -> str:
    """Return the resistor-color function name for *language*."""
    return "color_code" if language in SNAKE_CASE_LANGUAGES else "colorCode"
...
CANONICAL_CASES: list[dict] = load_canonical_cases(
    EXERCISE, property_filter="colorCode"
)
...
class TestResistorColorLowering:
...
class TestResistorColorCrossLanguage:
...
class TestResistorColorExecution:
    @pytest.fixture(
        params=[
            (lang, case)
    ...
    def execution_result(self, request):
        ...
        vm, stats = execute_for_language(lang, source)
