"""Exercism test: luhn across all 15 deterministic frontends.

Uses canonical test data from Exercism problem-specifications.
Each language solution is a separate file under exercises/luhn/solutions/.

Uses a charToDigit helper function with 10 if-statements mapping
digit characters to integers, and a two-pass algorithm for Luhn
validation (validate then compute checksum right-to-left).
"""
...
def _function_name(language: str) -> str:
    """Return the luhn function name for *language*."""
    return "is_valid" if language in SNAKE_CASE_LANGUAGES else "isValid"
...
class TestLuhnExecution:
    ...
    def execution_result(self, request):
        ...
        vm, stats = execute_for_language(lang, source, max_steps=10000)
