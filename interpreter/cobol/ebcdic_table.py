...
EBCDIC (Code Page 037 / CP500 common subset) and ASCII.
"""

from __future__ import annotations

# fmt: off
# EBCDIC-to-ASCII lookup: index = EBCDIC byte, value = ASCII byte.
_EBCDIC_TO_ASCII: tuple[int, ...] = (
    # 0x00–0x0F
...
def _build_reverse_table(forward: tuple[int, ...]) -> tuple[int, ...]:
    """Build ASCII→EBCDIC reverse lookup from EBCDIC→ASCII forward table."""
    reverse = [0x3F] * 256  # default to EBCDIC '?' for unmapped
    for ebcdic_val, ascii_val in enumerate(forward):
        # First EBCDIC value wins for each ASCII value
        if reverse[ascii_val] == 0x3F or ebcdic_val < reverse[ascii_val]:
            reverse[ascii_val] = ebcdic_val
    return tuple(reverse)


_ASCII_TO_EBCDIC: tuple[int, ...] = _build_reverse_table(_EBCDIC_TO_ASCII)
