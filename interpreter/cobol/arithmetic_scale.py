...
def expression_is_floating(node: ExprNode, field_types: FieldTypes) -> bool:
    ...
    if isinstance(node, BinOpNode):
        # Exponentiation is not lowered through the exact builtins yet
        # (red-dragon-wxur): IBM computes a fixed-point integer power as a
        # succession of multiplications, which this module does not model. It
        # is therefore computed in floating point, as it was before the exact
        # arithmetic work, rather than failing to lower at all.
        if node.op == "**":
            return True
