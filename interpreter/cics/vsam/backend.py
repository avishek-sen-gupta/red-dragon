"""VSAM persistence backends.

The engine keeps records in memory (SortedDict); a backend owns the durable copy
and is selected at engine instantiation. InMemoryBackend (default) starts empty
or from a pre-loaded seed dict. FileBackend keeps a durable write-through copy
in a backing directory; use seed_vsam() to pre-populate it before first run.
"""

from __future__ import annotations

from dataclasses import dataclass, field
from pathlib import Path
from typing import Protocol, runtime_checkable

from interpreter.cics.vsam.fct import DatasetConfig
from interpreter.cics.vsam.format import read_flat_file, write_flat_file


@runtime_checkable
class VsamBackend(Protocol):
    """Persistence boundary for the VSAM engine."""

    def load(self, name: str, cfg: DatasetConfig) -> list[bytes]:
        """Return the dataset's records (key order), or [] if none."""
        ...

    def persist(self, name: str, cfg: DatasetConfig, records: list[bytes]) -> None:
        """Durably store the dataset's records. May be a no-op (in-memory)."""
        ...


@dataclass
class InMemoryBackend:
    """Default: starts from an optional pre-seeded dict; never persists.

    Pass the result of load_vsam_seed() to pre-populate from EBCDIC flat files.
    """

    seed: dict[str, list[bytes]] = field(default_factory=dict)

    def load(self, name: str, cfg: DatasetConfig) -> list[bytes]:
        return self.seed.get(name.upper(), [])

    def persist(self, name: str, cfg: DatasetConfig, records: list[bytes]) -> None:
        return None


class FileBackend:
    """Write-through file persistence in a backing directory.

    load() reads from <backing_dir>/<NAME>.dat if it exists, else returns [].
    Use seed_vsam() to pre-populate the backing directory before first use.
    persist() writes the backing file after each mutation.
    """

    def __init__(self, backing_dir: Path) -> None:
        self._dir = Path(backing_dir)

    def _backing_path(self, name: str) -> Path:
        return self._dir / f"{name.upper()}.dat"

    def load(self, name: str, cfg: DatasetConfig) -> list[bytes]:
        backing = self._backing_path(name)
        if backing.exists():
            return read_flat_file(backing, cfg.record_length)
        return []

    def persist(self, name: str, cfg: DatasetConfig, records: list[bytes]) -> None:
        write_flat_file(self._backing_path(name), records, cfg.record_length)
