    ...
    for child in node.children:
        if child.type == PHPNodeType.PROPERTY_ELEMENT:
            ...
            if name_node:
                ...
                ctx.emit_inst(
                    LoadVar(result_reg=this_reg, name=VarName(constants.PARAM_PHP_THIS))
                )
                if value_node:
                    val_reg = ctx.lower_expr(value_node)
                else:
...
def lower_php_global_declaration(
    ...
    for child in node.children:
        if child.type == PHPNodeType.VARIABLE_NAME:
            ...
            reg = ctx.fresh_reg()
            ctx.emit_inst(LoadVar(result_reg=reg, name=VarName(var_name)), node=child)
            ctx.emit_inst(DeclVar(name=VarName(var_name), value_reg=reg), node=node)
...
def _extract_php_class(node) -> "tuple[str, ClassInfo] | None":
    ...
    if base_clause is not None:
        ...
        if parent_node is not None:
            parents = (ClassName(parent_node.text.decode()),)
