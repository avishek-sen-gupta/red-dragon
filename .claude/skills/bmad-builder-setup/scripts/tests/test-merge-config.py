...
class TestMergeConfig(unittest.TestCase):
    def test_fresh_install_with_core_and_module(self):
        ...
        self.assertEqual(
            result["bmb"]["bmad_builder_output_folder"], "_bmad-output/skills"
        )
    ...
    def test_module_metadata_always_from_yaml(self):
        ...
        self.assertEqual(result["bmb"]["name"], "BMad Builder")
        self.assertEqual(
            result["bmb"]["description"], "Standard Skill Compliant Factory"
        )
...
class TestEndToEnd(unittest.TestCase):
    def test_write_and_read_round_trip(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                written["bmb"]["bmad_builder_output_folder"], "_bmad-output/skills"
            )
...
class TestLoadLegacyValues(unittest.TestCase):
    def _make_legacy_dir(
        self, tmpdir, core_data=None, module_code=None, module_data=None
    ):
...
class TestCleanupLegacyConfigs(unittest.TestCase):
    def test_deletes_module_and_core_configs(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse(
                os.path.exists(os.path.join(legacy_dir, "core", "config.yaml"))
            )
            self.assertFalse(
                os.path.exists(os.path.join(legacy_dir, "bmb", "config.yaml"))
            )
    ...
    def test_leaves_other_module_configs_alone(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                os.path.exists(os.path.join(legacy_dir, "cis", "config.yaml"))
            )
...
class TestLegacyEndToEnd(unittest.TestCase):
    def test_full_legacy_migration(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                answers["module"]["bmad_builder_output_folder"], "new/skills"
            )
            self.assertEqual(
                answers["module"]["bmad_builder_reports"], "legacy/reports"
            )
