...
- **Pattern matching** — the shared Pattern ADT (`interpreter/frontends/common/patterns.py`) defines 10 pattern types: `LiteralPattern`, `WildcardPattern`, `CapturePattern`, `SequencePattern` (with `StarPattern` for `[first, *rest]`), `MappingPattern`, `ClassPattern` (positional via `__match_args__`), `OrPattern` (with capture bindings via mini linear chain), `AsPattern`, and `ValuePattern` (dotted constant lookup e.g. `Color.RED`). The `compile_match` compiler uses the CPython linear chain model with a two-pass test-before-bind design — all pattern tests run before any variable bindings, preventing partial binding on match failure. Guards are supported on all pattern types including recursive compound patterns. Python (`parse_pattern` in `interpreter/frontends/python/patterns.py`) and C# (`parse_csharp_pattern` in `interpreter/frontends/csharp/patterns.py`) are the first two consumers. No new opcodes or VM changes were required — pattern matching decomposes into existing primitives (`BINOP ==`, `CALL_FUNCTION isinstance/len`, `LOAD_INDEX`, `LOAD_FIELD`, `STORE_VAR`, `BRANCH_IF`). The `isinstance` builtin is enhanced to handle both heap objects (class registry lookup) and primitive types (`int`, `str`, `bool`, `float`, `list`)
...
The pipeline result also includes **interprocedural analysis** (call graph, function summaries, and whole-program dependency graph) when the source contains function definitions. The **Dataflow Summary** panel (`viz/panels/dataflow_summary_panel.py`) renders this as a collapsible tree: each function shows its callers, callees, and merged data-flow summaries across call contexts. The **Dataflow Graph** panel (`viz/panels/dataflow_graph_panel.py`) renders the whole-program flow graph as annotated edges grouped by source endpoint, with register-to-call-result annotations for improved readability. It also provides `find_top_level_call_sites` to discover call instructions in top-level (non-function) code, along with `TopLevelCall` and `ChainNode` data models for building call-chain tree views. `build_call_chain` recursively constructs a per-parameter flow tree for a function: each parameter's data flows to return endpoints, field writes, or through inner call sites (with argument mapping) into recursive subtrees, with cycle detection via immutable visited sets.
