...
- **Pre-lowering symbol extraction** — all 15 tree-sitter frontends now run a Phase 2 symbol extraction pass before IR lowering, building a `SymbolTable` of `ClassInfo` (fields, methods, constants, parents) for every class/struct declaration in the source. OOP languages (JavaScript, TypeScript, Ruby, Kotlin, Scala, PHP) extract class fields, methods, and inheritance chains. Struct-based languages (Go, C, Rust) extract struct fields and top-level functions as `ClassInfo` and `FunctionInfo` respectively. Go methods are attached to their receiver struct. Pascal classes (`declType > declClass`) extract fields from `declSection > declField` nodes. Lua returns an empty symbol table (table-based OOP has no fixed class syntax). The symbol table is available to the lowering pass for field resolution, type seeding, and constructor generation. `resolve_field` walks the class hierarchy for inherited fields and returns a `NULL_FIELD` sentinel (not `None`) when no field is found. The COBOL frontend uses `from_data_layout` to construct a compatible `SymbolTable` from its `DataLayout`
- **Implicit-this field reads** — in Java, C#, and C++ instance methods, bare identifier reads are checked against `resolve_field` in the symbol table and `_method_declared_names` (variables declared so far in the current method). If the identifier names a class field that is not shadowed by a local parameter or variable, a `LOAD_FIELD this` is emitted instead of a plain `LOAD_VAR`. This complements the existing implicit-this field store. `reset_method_scope()` is called at method entry to clear `_method_declared_names`; the `emit()` hook updates it for every `DECL_VAR`/`STORE_VAR` emitted during lowering
- **Pattern matching** — the shared Pattern ADT (`interpreter/frontends/common/patterns.py`) defines 13 pattern types: `LiteralPattern`, `WildcardPattern`, `CapturePattern`, `SequencePattern` (with `StarPattern` for `[first, *rest]`), `MappingPattern`, `ClassPattern` (positional via `__match_args__`), `OrPattern` (with capture bindings via mini linear chain), `AsPattern`, `ValuePattern` (dotted constant lookup e.g. `Color.RED`), `DerefPattern` (Rust `&val`), `RelationalPattern` (C# `> 5`, `< 10`), `AndPattern` (C# `> 0 and < 10`), and `NegatedPattern` (C# `not 0`). The `compile_match` compiler uses the CPython linear chain model with a two-pass test-before-bind design — all pattern tests run before any variable bindings, preventing partial binding on match failure. Guards are supported on all pattern types including recursive compound patterns. Seven consumers: Python (`parse_pattern`), C# (`parse_csharp_pattern` — constant, declaration, recursive, or, list, parenthesized, relational, and, negated patterns), Java (`parse_java_pattern` — type, record, wildcard patterns + guards), Rust, Ruby, Kotlin, and Scala. No new opcodes or VM changes were required — pattern matching decomposes into existing primitives (`BINOP ==`, `CALL_FUNCTION isinstance/len`, `LOAD_INDEX`, `LOAD_FIELD`, `STORE_VAR`, `BRANCH_IF`). The `isinstance` builtin is enhanced to handle both heap objects (class registry lookup) and primitive types (`int`, `str`, `bool`, `float`, `list`)
...
| `analyze_project(entry_file, language, project_root?)` | `InterproceduralResult` | Multi-file: discover → compile → link → interprocedural analysis |
| `run_project(entry_file, language, project_root?, max_steps?, verbose?)` | `VMState` | Multi-file: discover → compile → link → execute |
...
result = analyze_project("src/main.py", Language.PYTHON)
# result.call_graph — cross-module call graph
# result.summaries — per-function dataflow summaries
# result.whole_program_graph — whole-program dependency graph

# Multi-file execution
vm = run_project("src/main.py", Language.PYTHON, max_steps=500)
frame = vm.call_stack[0]
print(frame.local_vars["result"])  # concrete value from cross-module execution
```
...
|----------|---------------------|
| **Python** | list/dict/set comprehensions (including nested), generator expressions, with statements, decorators, lambdas, yield/await, assert, import/from-import, walrus operator (`:=`), match statements with structural pattern matching (3.10+ — literal, wildcard, capture, sequence, mapping, class, or, as patterns + guards, star/splat patterns `[first, *rest]` including star-at-start, star-at-end, star-in-middle, and nested star patterns, value patterns via dotted constant lookup `case Color.RED:` and multi-level `case HTTP.Status.OK:`), delete, slicing, f-string interpolation, list/dict/splat patterns, ellipsis (`...`), splat/spread (`*args`, `**kwargs` in expressions), expression lists, dotted names |
...
| **Ruby** | symbols, ranges, regex, lambdas, string/symbol arrays (`%w`/`%i`), case/when, case/in structural pattern matching via Pattern ADT (`parse_ruby_pattern` — literal int/str/float, wildcard `_`, capture, `alternative_pattern` → OrPattern, `array_pattern` → SequencePattern with `*rest` StarPattern, `hash_pattern` → MappingPattern with keyword shorthand `{name:}`, `as_pattern` `X => binding`, `constant` → ClassPattern, `Point[x, y]` class pattern with positional sub-patterns, unary negative literals `-1`), modules, global/class variables, heredocs, if/unless/while/until modifiers, ternary operator, unary operators, `self`, singleton classes/methods, element reference (array indexing), string interpolation (`"Hello #{expr}"`), heredoc interpolation (`<<~HEREDOC\nHello #{expr}\nHEREDOC`), hash key symbols, `super`, `yield`, delimited symbols (`:"dynamic"`), `retry`, class instantiation (`Counter.new()`), instance variable field semantics (`@var` → `LOAD_FIELD`/`STORE_FIELD`), `initialize` → `__init__` constructor mapping, scope resolution (`Module::Class`), rescue modifier (`expr rescue fallback`), array slicing (`arr[1..3]` inclusive, `arr[1...3]` exclusive, `arr[start, length]` positional) |
| **PHP** | switch, do-while, match expressions, arrow functions, scoped calls (`Class::method`), namespaces, interfaces, traits, enums, static variables, goto/labels, anonymous functions (closures), null-safe access (`?->`), null-safe method calls (`?->method()`), class constant access, property declarations, yield, heredoc/nowdoc, string interpolation (`"Hello $name"` / `"Hello {$expr}"` / heredoc interpolation), relative scope (`self::`/`static::`), dynamic variable names (`$$x`), global declarations, include/require_once expressions, variadic unpacking (`...$arr`), `print` intrinsic, `clone` expressions (shallow heap copy), `const` declarations, error suppression (`@expr`), `exit`/`die`, `declare`, `unset`, `list()` destructuring (`list($a, $b) = $arr`), `strlen` builtin, anonymous classes (`new class { ... }`) |
| **Java** | records, instanceof with type patterns and record patterns (Java 16+: `o instanceof String s` binding, `o instanceof Point(int x, int y)` record destructuring), method references (`Type::method`), lambdas, class literals (`Type.class`), do-while, assert, labeled statements, synchronized blocks, static initializers, explicit constructor invocations (`super()`/`this()`), annotation type declarations, scoped identifiers (`java.lang.System`), switch expressions (Java 14+, including expression_statement, throw in arms, `yield` in block arms, and pattern matching with type patterns, guards (`when` clauses), and wildcard `_`), hex floating point literals (`0x1.0p10`) |
| **C#** | await, yield, switch expressions (C# 8) with pattern matching (constant, declaration/var, recursive/property, or, list, parenthesized, relational `> 5`, and `> 0 and < 10`, negated `not 0` patterns), lock, using statements, checked/fixed blocks, events, typeof, is-check, property declarations, lambdas, null-conditional access (`?.`), local functions, tuples, is-pattern expressions, declaration patterns, array initializer expressions, string interpolation (`$"Hello {expr}"` — format specifiers and alignment clauses are discarded as presentation-only), record declarations, verbatim strings (`@"..."`), constant patterns, delegate declarations, implicit object creation (`new()`), LINQ query expressions (`from...where...select`), throw expressions (`x ?? throw new ...`), goto/labeled statements, `default` expressions, `sizeof` expressions, `checked`/`unchecked` expressions, `out`/`ref`/`in` parameter modifiers with pass-by-reference semantics (callee writes propagate to caller via ADDRESS_OF/Pointer infrastructure), anonymous object creation (`new { Name = "foo", Age = 42 }`) |
