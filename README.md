...
- **Pre-lowering symbol extraction** — all 15 tree-sitter frontends now run a Phase 2 symbol extraction pass before IR lowering, building a `SymbolTable` of `ClassInfo` (fields, methods, constants, parents) for every class/struct declaration in the source. OOP languages (JavaScript, TypeScript, Ruby, Kotlin, Scala, PHP) extract class fields, methods, and inheritance chains. Struct-based languages (Go, C, Rust) extract struct fields and top-level functions as `ClassInfo` and `FunctionInfo` respectively. Go methods are attached to their receiver struct. Pascal classes (`declType > declClass`) extract fields from `declSection > declField` nodes. Lua returns an empty symbol table (table-based OOP has no fixed class syntax). The symbol table is available to the lowering pass for field resolution, type seeding, and constructor generation
