...
**Runtime coercion** — An immutable `TypeEnvironment` drives write-time coercion during VM execution via pluggable `TypeConversionRules`. The default rules handle Int/Float promotion, division semantics, and Bool→Int coercion. Languages with different semantics (e.g. COBOL fixed-point arithmetic) can supply their own subclass. Raw Python values are wrapped in `TypedValue(value, type)` — an immutable dataclass pairing each value with its `TypeExpr` — enabling type-aware operations throughout the VM. Factory helpers `typed()` and `typed_from_runtime()` construct `TypedValue` instances with explicit or inferred types respectively.
