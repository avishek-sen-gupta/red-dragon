...
**Static type inference** — `infer_types()` accepts the pre-seeded builder and propagates types through register/variable chains across 15 opcodes (CONST through RETURN). The pass runs to fixpoint, resolving forward references where a function calls another function defined later in the IR. The entire type pipeline operates on `TypeExpr` objects end-to-end: frontends call `parse_type()` at the seeding boundary, `TypeEnvironmentBuilder` stores `TypeExpr`, the inference engine and type resolver work with `TypeExpr` throughout, and `UNKNOWN` (falsy `UnknownType` singleton) replaces empty strings. No string serialization/deserialization roundtrips anywhere in the pipeline. The inference pass handles:
...
- **Function types** — `FunctionType(params, return_type)` represents callable types with `Fn(Int, String) -> Bool` syntax. Parser handles `parse_type("Fn(Int, String) -> Bool")` round-trips. TypeGraph supports function subtyping with contravariant parameters and covariant return types. The inference engine assigns `FunctionType` to CONST function references when parameter/return types are known, and CALL_UNKNOWN resolves return types from `FunctionType` targets
- **Tuple types** — Python tuple literals `(1, "hello")` produce `Tuple[Int, String]` via `ParameterizedType("Tuple", ...)`. Element types are tracked per-index, so `t[0]` on a `Tuple[Int, String]` resolves to `Int` while `t[1]` resolves to `String`. TypeGraph supports covariant element subtyping and pairwise LUB
- **Type aliases** — `typedef int UserId;` (C), `type StringMap = Map<string, string>` (TS), etc. are seeded as type aliases and resolved transitively at inference time. Alias resolution expands `UserId` → `Int`, `IntPtr` → `Pointer[Int]`, and chains like `Km` → `Distance` → `Int`. Aliases are available in the final `TypeEnvironment` for inspection
- **Interface/trait typing** — Java `implements` clauses are extracted from tree-sitter ASTs and stored as `interface_implementations` in `TypeEnvironment`. `TypeGraph.extend_with_interfaces()` adds class→interface edges so `Dog ⊆ Comparable` subtype checks work. Interface nodes are added with `kind="interface"` and parented under `Any`
- **Variance annotations** — `TypeGraph` supports a variance registry mapping constructor names to per-argument variance (`Variance.COVARIANT`, `CONTRAVARIANT`, `INVARIANT`). Unlisted constructors default to all-covariant (backwards-compatible). `MutableList` with `INVARIANT` blocks `MutableList[Int] ⊆ MutableList[Number]`. Mixed variance supported (e.g., `Map` with invariant key, covariant value)
- **Bounded type variables** — `TypeVar(name, bound)` represents generic type parameters like Java's `<T extends Number>`. Unbounded TypeVars default to `Any`. Subtype rules: a concrete type satisfies `T: Number` iff it's a subtype of `Number`; a TypeVar child is a subtype of a parent iff its bound is. TypeVars compose with parameterized types (`Array[Int] ⊆ Array[T: Number]` via covariant checking)
...
- **Function-scoped variable types** — variable types are tracked per function scope, preventing cross-function name collisions (e.g. `x = 42` in one function and `x = "hello"` in another infer independently). Global-scope variables are visible as fallback inside functions. Per-function scoped types are exposed in `TypeEnvironment.scoped_var_types`, and the flat view merges conflicting names via `union_of` (not last-writer-wins)
- **Block-scope tracking (LLVM-style)** — 9 block-scoped frontends (Java, C, C++, C#, Rust, Go, Kotlin, Scala, TypeScript) set `BLOCK_SCOPED = True`, enabling automatic scope entry/exit in `lower_block()` and variable name mangling via `declare_block_var()`. When a variable shadows an outer scope, the frontend mangles the name (e.g. `x` → `x$1`) and records `VarScopeInfo(original_name, scope_depth)` metadata. `resolve_var()` in `lower_identifier()` and `lower_store_target()` resolves reads/writes through the scope stack. Loop variables (enhanced-for, range-for, for-in/of, foreach) and catch clause variables also enter their own block scopes, so `for (int x : list)` correctly shadows an outer `x`. Function-scoped languages (Python, JavaScript `var`, Ruby, Lua, PHP, Pascal) bypass this entirely (`BLOCK_SCOPED = False`). Metadata is carried through `TypeEnvironmentBuilder` to the final `TypeEnvironment.var_scope_metadata`
...
| **C** | pointer dereference/address-of, function pointers (`int (*fp)(int,int) = &add; (*fp)(3,5)`) including function-pointer return types (`int (*get_op(int))(int,int)`), struct pointers with arrow operator (`p->x`, pass-by-pointer, linked structures), sizeof, compound literals, struct/union/enum definitions, initializer lists, designated initializers (`.field = value`), goto/labels, typedef, char literals, function-like macros (`#define MAX(a, b) ...`) |
