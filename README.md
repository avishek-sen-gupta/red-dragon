...
**Scoping** — Variable types are tracked per function scope, preventing cross-function name collisions. 9 block-scoped frontends (Java, C, C++, C#, Rust, Go, Kotlin, Scala, TypeScript) use LLVM-style name mangling (`x` → `x$1`) to disambiguate shadowed variables in nested blocks, loops (enhanced-for, range-for, for-in/of, foreach), catch clauses, and C-style for-loop init declarations (`for(int i = 0; ...)`). `VarScopeInfo` metadata preserves original names and scope depths. Function-scoped languages (Python, JavaScript `var`, Ruby, etc.) bypass this entirely.
