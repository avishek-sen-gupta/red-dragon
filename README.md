...
- **Block-scope tracking (LLVM-style)** — `TreeSitterEmitContext` provides `enter_block_scope()`, `exit_block_scope()`, `declare_block_var()`, and `resolve_var()` for block-scoped frontends. When a variable shadows an outer scope, the frontend mangles the name (e.g. `x` → `x$1`) and records `VarScopeInfo(original_name, scope_depth)` metadata. Function-scoped languages bypass this entirely. Metadata is carried through `TypeEnvironmentBuilder` to the final `TypeEnvironment.var_scope_metadata`
