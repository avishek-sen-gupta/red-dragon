...
- **Executes** programs via a deterministic VM — tracking data flow through incomplete programs with missing imports or unknown externals entirely without LLM calls — with **type-aware register resolution** (`_resolve_typed_reg`) that coerces runtime values to their statically-inferred types using `TypeEnvironment` and `ConversionRules` (e.g. Float→Int via `math.trunc` for array indices computed by division), and a configurable **LLM plausible-value resolver** that can replace symbolic placeholders with concrete values for unresolved function/method calls
