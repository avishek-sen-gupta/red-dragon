...
<details>
<summary><strong>VM feature implementation details</strong> (click to expand)</summary>
...
- **Implicit-this field store** — in constructors, bare `x = v` assignments where `x` is a declared class field are rewritten to `LOAD_VAR this` + `STORE_FIELD` at lowering time. **Implicit-this field reads** — `resolve_field` walks the class hierarchy; `_method_declared_names` tracks local variables to avoid shadowing. `reset_method_scope()` called at method entry.
