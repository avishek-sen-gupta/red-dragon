...
All 15 tree-sitter frontends follow a modular **pure functions + context** architecture. Each frontend is a package (`interpreter/frontends/<lang>/`) containing:

- **`frontend.py`** — thin orchestrator class inheriting from `BaseFrontend`, overriding only `_build_constants()`, `_build_stmt_dispatch()`, and `_build_expr_dispatch()` to assemble dispatch tables from pure functions
- **`expressions.py`** — language-specific expression lowerers as pure functions `(ctx: TreeSitterEmitContext, node) -> str`
- **`control_flow.py`** — language-specific control flow lowerers
- **`declarations.py`** — language-specific declaration lowerers

Shared lowerers live in `interpreter/frontends/common/` (expressions, control_flow, declarations, assignments, exceptions). The `TreeSitterEmitContext` dataclass (`interpreter/frontends/context.py`) holds all mutable state (registers, labels, instructions, loop stacks) and utility methods, injected as the first argument to every pure-function lowerer. C++ extends C's frontend and TypeScript extends JavaScript's — both call `super()._build_*()` and overlay their own entries. The COBOL frontend uses a separate but analogous `EmitContext`-based decomposition.
...
**8469 tests** (8402 unit + 67 integration passed, 4 skipped, 21 xfailed) — all with zero LLM calls.
