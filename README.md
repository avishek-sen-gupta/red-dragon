...
RedDragon has a three-phase type system: **frontend extraction**, **static inference**, and **runtime coercion**. See the full [Type System Design Document](docs/type-system.md) for architecture, algorithms, and per-opcode inference rules.

**Frontend type extraction** — 13 statically-typed frontends extract type annotations from tree-sitter ASTs during lowering and seed them into a `TypeEnvironmentBuilder`. Generic types (e.g. `List<Map<String, Integer>>`) are extracted structurally from AST nodes and normalised recursively through per-frontend type maps. All types are represented as `TypeExpr` algebraic data types (`ScalarType`, `ParameterizedType`, `UnionType`, `FunctionType`, `TypeVar`, `UnknownType`) with no string roundtrips in the pipeline. Class declarations and interfaces seed `Type[ClassName]` metatype entries, enabling the type system to track constructor-valued variables.

**Static type inference** — `infer_types()` propagates types through register/variable chains across 15 opcodes to fixpoint. Key capabilities: self/this class typing, function return type backfilling, union type widening, class field tracking, parameterized type promotion (e.g. `Array[Int]`), function types with contravariant/covariant subtyping, tuple element types, type alias resolution, interface/trait hierarchies, bounded type variables, class-scoped method signatures with overload support, and 60+ builtin method return types. All signatures live in a unified `method_signatures` container keyed by `TypeExpr` — class methods under their class type (e.g. `ScalarType("Dog")`), standalone functions under the `UNBOUND` sentinel — preventing cross-class name collisions. Each `FunctionSignature` carries a `FunctionKind` (UNBOUND/INSTANCE/STATIC) and a `callable_params` property that excludes the implicit `this` receiver.

**Scoping** — Variable types are tracked per function scope, preventing cross-function name collisions. 9 block-scoped frontends (Java, C, C++, C#, Rust, Go, Kotlin, Scala, TypeScript) use LLVM-style name mangling (`x` → `x$1`) to disambiguate shadowed variables in nested blocks, loops (enhanced-for, range-for, for-in/of, foreach), catch clauses, C-style for-loop init declarations (`for(int i = 0; ...)`), Go if/switch init statements (`if x := expr; cond`), C++17 if-init (`if (int x = expr; cond)`), Java try-with-resources (`try(var r = expr)`), C# using declarations (`using(var r = expr)`), and Kotlin when-subject bindings (`when(val x = expr)`). `VarScopeInfo` metadata preserves original names and scope depths. Function-scoped languages (Python, JavaScript `var`, Ruby, etc.) bypass this entirely.

**Runtime coercion** — An immutable `TypeEnvironment` drives write-time coercion during VM execution via pluggable `TypeConversionRules`. The default rules handle Int/Float promotion, division semantics, and Bool→Int coercion. Languages with different semantics (e.g. COBOL fixed-point arithmetic) can supply their own subclass. The execution loop uses a split update pipeline: local executor handlers produce `TypedValue`-wrapped register writes, variable writes, and return values directly, with `coerce_local_update` applying only register type coercion (e.g. Float→Int) to match declared types. Return values use three-state semantics via `TypedValue`: `typed(None, scalar("Void"))` for void returns (no operands), `typed(None, UNKNOWN)` for explicit `return None`, and `typed(value, inferred_type)` for concrete returns. LLM responses go through `materialize_raw_update`, which deserializes dicts into typed objects (`SymbolicValue`, `Pointer`), applies register coercion, wraps each value in `TypedValue(value, type)`, and materializes raw return values. Then `apply_update` stores the pre-materialized `TypedValue` entries into registers and local variables (both typed as `dict[str, TypedValue]`), with a lightweight coercion check for declared-type mismatches. All VM storage — `StackFrame.registers`, `StackFrame.local_vars`, `HeapObject.fields`, and `ClosureEnvironment.bindings` — stores `TypedValue` exclusively with no raw-value fallback guards. `HeapObject.type_hint` uses `TypeExpr` (default `UNKNOWN`) instead of raw strings, enabling parameterized type hints like `Box[Node]` while retaining string compatibility via `TypeExpr.__eq__`. The `_resolve_reg` helper auto-unwraps `TypedValue` for opcode handlers that need raw values, while `_resolve_binop_operand` preserves the full `TypedValue` wrapper — used by BINOP and all three call handlers (`CALL_FUNCTION`, `CALL_METHOD`, `CALL_UNKNOWN`). All builtins (including byte builtins and method builtins) receive `list[TypedValue]` args and unwrap `.value` internally at each access point; the overload resolver receives `list[TypedValue]` args directly for type-aware dispatch (exact match, coercion, and subtype scoring via `TypeGraph`), while `UnresolvedCallResolver` still receives unwrapped `list[Any]`. Factory helpers `typed()` and `typed_from_runtime()` construct `TypedValue` instances with explicit or inferred types respectively. Binary operations use an injectable `BinopCoercionStrategy` protocol — `DefaultBinopCoercion` (no-op) for most languages, `JavaBinopCoercion` (auto-stringifies `String + non-String`) for Java — enabling language-correct operator semantics without modifying the core operator table. BaseFrontend exposes a `_emit_prelude` hook (no-op default) that subclasses can override to emit IR class definitions before user code; the Rust frontend uses this to emit `Box` and `Option` prelude classes with `__init__`, `unwrap`, and `as_ref` methods, enabling `Box::new(v)` and `Some(v)` to resolve to real class constructors at runtime. A Rust-specific call lowerer intercepts `Box::new(expr)` as a pass-through (returning the argument directly, since all VM values are already reference-based) and `Some(expr)` as `CALL_FUNCTION "Option"`, enabling `Option` method dispatch (`.unwrap()`, `.as_ref()`) while making `Box` transparent — matching Rust's auto-deref semantics. This enables the Rosetta linked list traversal (`node.next_node.as_ref().unwrap()`) to produce concrete results across `Option<Box<Node>>` chains.
...
- **Field initializer lowering** — instance field initializers (e.g. `int count = 0` in Java, `var count: Int = 0` in Kotlin, `public $count = 0` in PHP) are lowered as `STORE_FIELD` instructions prepended to every constructor body, matching how real compilers (javac, Roslyn, kotlinc, scalac) handle them. Classes without explicit constructors get a synthetic constructor (`__init__` or `__construct` for PHP). This ensures heap object fields are properly populated at construction time, enabling correct method chaining (`obj.method().method()`) across Java, C#, Kotlin, Scala, and PHP. C++ `*this` dereferences in return statements are handled transparently, passing through the `this` reference without spurious `LOAD_FIELD`
...
| **C++** | lambda expressions (closures), field initializer lists, delete expressions, enum class, array subscript expressions, C++20 concepts, constructor field storage (VM-injected `this`) |
...
| **Lua** | anonymous functions, varargs, goto/labels, method calls (`obj:method()`) |
| **Pascal** | nested functions, field access, array indexing, unary operators, case-of, repeat-until, set literals, const/type/uses declarations, parenthesized expressions, try/except/finally, exception handlers (`on E: Exception do`), raise, ranges (`4..10`), with statements, inherited calls, for-in loops, goto/labels, class declarations |
...
- **Classes** — heap allocation, method dispatch with overload resolution (arity + type + subtype-aware scoring via TypeGraph), field access
