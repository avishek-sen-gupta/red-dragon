...
| **C** | pointer dereference/address-of, function pointers (`int (*fp)(int,int) = &add; (*fp)(3,5)`), sizeof, compound literals, struct/union/enum definitions, initializer lists, designated initializers (`.field = value`), goto/labels, typedef, char literals, function-like macros (`#define MAX(a, b) ...`) |
